...
-   When the outcome is categorical with more than 2 categories, we use the multinomial probit regression. We will focus on simple probit regression here
...
## Our Implementation of the Probit Function
...
-   We are currently working on estimating the standard errors using bootstrapping
...
-   We were a bit unsure of which method of optimization to use. We decided to use optim as it proved to be user-friendly for us
...
```{r}
fit_probit <- glm(admit ~ gpa, data = my_data,
                 family = binomial(link = 'probit'))
summary(fit_probit)$coef
```

-   Interpreting the coefficient of gpa: Comparing 2 groups of people, one who got admitted and one who did not, the average gpa of those who did get admitted is around 0.63 z-scores higher than the group which did not

## AUC using the correct model
...
predicted_probit_correct <- predict(fit_probit, test_probit, type='response')
...
## AUC using the wrong model (breaking the linearity assumption)

-   Checking if the z-score of the outcome and predictor have a linear relationship

```{r}
my_data <- my_data %>% mutate(gpa_wrong = gpa^2)
fit_probit_wrong <- glm(admit ~ gpa_wrong, data = my_data,
                 family = binomial(link = 'probit'))
train_indices <- sample(x = nrow(my_data), size = 320, replace = F)
test_indices <- c(1:400)[-train_indices]
train_probit <- my_data[train_indices,]
test_probit <- my_data[test_indices,]
predicted_probit_incorrect <- predict(fit_probit_wrong, test_probit, type='response')
...
```{# originally, the relationship looked linear}
ggplot(my_data, aes(x = gpa, y = my_data$admit)) +
      geom_point() +
      geom_smooth(method = "loess", se = FALSE) +
      theme_classic()

my_data$probabilities <- fit_probit$fitted.values
my_data <- my_data %>%
    mutate(probit = qnorm(probabilities))
ggplot(my_data, aes(y = probit, x = gpa))+
...
## Trying to break the linearity assumption
